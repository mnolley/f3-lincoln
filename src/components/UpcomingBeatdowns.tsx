...
export function UpcomingBeatdowns({ slots, error }: Props) {
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-ink-muted">
            ...
            has a claim in Slack; otherwise the slot is open.
          </p>
        </div>
        {site.slackUrl ? (
          <a
      ...
      {slots.length === 0 ? (
        ...
        <ol className="grid gap-4 lg:grid-cols-3">
          {slots.map((slot, i) => (
            <li key={`${slot.ymd}-${slot.ao}`} className="card-panel flex flex-col p-5 sm:p-6">
